        ...
        */
        ...
        _loginUser: function (e) {
            ...
            var username = this._changeCase(this.usernameInputNode.value),
				...
				token = this.tokenInputNode.value;
            ...
            if (username && password && token) {
				...
				this._twoFactorLogin(username, password, token, dojoLang.hitch(this, this._loginSuccess),dojoLang.hitch(this, this._loginFailed));
			} else if (username && password) {
				if (this.showprogress) {
					logger.debug("Showing Progress!!");
					this._indicator = mx.ui.showProgress();
				}
				this._singleFactorLogin(username, password, dojoLang.hitch(this, this._loginSuccess), dojoLang.hitch(this, this._showStep2), dojoLang.hitch(this, this._loginFailed));
            } else {
                domClass.remove(this.alertMessageNode, "hidden");
		...
		_singleFactorLogin: function (userString, passwordString, successCallback, codeSentCallback, failCallback) {
			var parameters = {user:userString,password:passwordString};
				...
				load: function(data, ioargs){
					...
					switch(ioargs.xhr.status){
						...
						default:
							...
							failCallback(ioargs.xhr.status);
					}
				},
		...
		_twoFactorLogin: function (userString, passwordString, tokenString, successCallback, failCallback) {
			var parameters = {user: userString,password: passwordString, token: tokenString};
		...
		_showStep1: function() {
			domClass.add(this.step2Node, "hide");
			domClass.remove(this.step1Node, "hide");
		},
		_showStep2: function() {
			domClass.add(this.step1Node, "hide");
			domClass.remove(this.step2Node, "hide");
		},
